...
TEST_CASE("GameSettingsConfig: save and load round-trip every field", "[Settings][GameSettings]")
{
    ...
    CHECK(dst.activeMods == src.activeMods);
...
TEST_CASE("GameSettingsConfig: persists active mods across the same game.cfg as the active profile",
          "[Settings][GameSettings]")
{
    ...
    CHECK(Poseidon::LoadActiveMods().empty());

    Poseidon::SaveActiveMods("@CWA;@Extra");
    CHECK(Poseidon::LoadActiveMods() == "@CWA;@Extra");

    // Saving the active profile afterwards must not clobber the mods list (and vice
    // versa) — both fields round-trip through the same game.cfg independently.
    Poseidon::SaveActiveProfile("Veteran");
    CHECK(Poseidon::LoadActiveMods() == "@CWA;@Extra");
    CHECK(Poseidon::LoadActiveProfile() == "Veteran");
