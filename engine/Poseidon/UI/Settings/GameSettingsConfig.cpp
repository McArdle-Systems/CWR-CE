...
bool GameSettingsConfig::Load(const std::string& path)
{
    ...
    if (auto* entry = cfg.FindEntry("activeMods"))
        activeMods = ((RString)*entry).Data();
...
bool GameSettingsConfig::Save(const std::string& path) const
{
    ...
    cfg.Add("activeMods", RString(activeMods.c_str()));
    cfg.Add("blood", blood);
...
{
    GameSettingsConfig cfg;
    if (cfg.Load(GameSettingsPath()))
        return cfg.activeMods;
    return {};
}
...
void SaveActiveMods(const std::string& modPath)
{
    ...
    if (!EnsureGameSettingsFile(cfg, path, env))
    {
        LOG_WARN(Config, "GameSettings: failed to prepare '{}' for active mods", path);
        return;
    }

    cfg.activeMods = modPath;
    if (!cfg.Save(path))
        LOG_WARN(Config, "GameSettings: failed to save active mods to '{}'", path);
}
