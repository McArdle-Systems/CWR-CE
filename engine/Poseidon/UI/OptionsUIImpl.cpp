...
void DisplaySingleMission::OnChangeMission()
{
    ...
    else
    {
        ...
        if (missionType == 1)
        {
            // uniquePrefix=true: each packed mission's bank is extracted under a
            // name derived from its own filename, not the shared per-island
            // prefix -- switching the selection between two packed missions on
            // the same island previously reused one bank's stale extraction.
